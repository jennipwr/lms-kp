...
<div class="kelas-wrapper">
    <div class="kelas-card">
        ...
        <div class="kelas-header">
            ...
            <h1 class="header-title">Tambah Kelas Baru</h1>
            <p class="header-sub">Isi formulir di bawah untuk membuat kelas dan menambahkan peserta</p>
        </div>
            ...
            <form action="{{ route('admin.store-kelas') }}" method="POST">
                ...
                <div class="field-group">
                    ...
                    <select name="kelas_label" class="field-input" required>
                        ...
                        <option value="A" {{ old('kelas_label') == 'A' ? 'selected' : '' }}>A</option>
                        <option value="B" {{ old('kelas_label') == 'B' ? 'selected' : '' }}>B</option>
                        <option value="C" {{ old('kelas_label') == 'C' ? 'selected' : '' }}>C</option>
                    </select>
                </div>
                ...
                <div class="field-group">
                    ...
                    <div style="display:flex; gap:8px; align-items:center;">
                        <input type="text" name="join_token" id="joinTokenInput" class="field-input"
                            ...
                            value="{{ old('join_token', $generated_token) }}"
                ...
                <div class="field-group">
                    ...
                    @if($dosen->isEmpty())
                        <div class="no-dosen-warn">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                            Belum ada data dosen. Tambahkan dosen terlebih dahulu.
                        </div>
                    @else
                        <select name="dosen_nik" class="field-input" required>
                            <option value="" disabled {{ old('dosen_nik') ? '' : 'selected' }}>— Pilih Dosen —</option>
                            @foreach($dosen as $d)
                                <option value="{{ $d->nik }}" {{ old('dosen_nik') == $d->nik ? 'selected' : '' }}>
                                    {{ $d->user->nama }} ({{ $d->nik }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="field-group">
                    ...
                    <div class="picker-box" id="mahasiswaPicker">
                        {{-- Search bar --}}
                        ...
                        <div class="picker-list" id="pickerList">
                            @forelse($mahasiswa as $m)
                                @php $checked = collect(old('mahasiswa', []))->contains($m->nrp); @endphp
                ...
                <div class="btn-row">
                    <button type="submit" class="btn-save" {{ $dosen->isEmpty() ? 'disabled' : '' }}>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
                        Simpan Kelas
                    </button>
...
<style>
    ...
    .picker-cb input:checked ~ .cb-box {
        background: var(--mcu-blue);
        border-color: var(--mcu-blue);
    }
